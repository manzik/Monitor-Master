...
monitor = new (function ()
{
    ...
    try
    {
        fs.openSync(__dirname+"/sites.json", 'r', function (err, fd)
        {
        });
    }
    catch (e)
    {
        fs.writeFileSync(__dirname+"/sites.json", "{\"sites\":[]}", {}, function (err)
        {
            if (err) alert(err);
        });
    }
    try
    {
        fs.openSync(__dirname+"/notify.json", 'r', function (err, fd)
        {
        });
    }
    catch (e)
    {
        fs.writeFileSync(__dirname+"/notify.json", "{\"sites\":{},\"default\":{\"nwo\":true,\"nbo\":true,\"email\":\"\",\"baloon\":true}}", {}, function (err)
        {
            if (err) alert(err);
        });
    }
    ...
    this.init = function ()
    {
        ...
        getsites(function (l)
        {
            ...
            arr_sites.unshift([]);
            childs.push(cp.fork(__dirname+"/monitorchild.js", [], { cwd: __dirname }));
        ...
        fs.readFile(__dirname+"/notify.json", "utf8", function (err, data)
        {
    ...
    this.setnotify=function(url,protocol,email,nwo,nbo,baloon)
    {
        ...
        fs.writeFile(__dirname+'/notify.json', JSON.stringify(notifyoptions), function (err)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < data.sites.length; i++)
                {
                    if (data.sites[i].url == url && data.sites[i].protocol == protocol)
                    {
                        ...
                        fs.writeFile(__dirname+'/sites.json', JSON.stringify(data), function (err)
                        {
